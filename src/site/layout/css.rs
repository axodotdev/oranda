...
fn fetch_css(dist_dir: &str, tag: &str) -> Result<String> {
    match env::var("ORANDA_CSS") {
        ...
        Err(_) => {
            let filename = format!("oranda-{tag}.css");
            let dest_path = Utf8Path::new(dist_dir).join(&filename);
            let oranda_css_response =
                tokio::runtime::Handle::current().block_on(fetch_oranda(tag))?;
...
async fn fetch_oranda(tag: &str) -> Result<String> {
