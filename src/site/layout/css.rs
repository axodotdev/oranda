...
pub fn place_css(dist_dir: &str, release_tag: &str) -> Result<()> {
    let maybe_env = option_env!("ORANDA_CSS_EXISTS");
    ...
    if cfg!(debug_assertions) && release_tag == ORANDA_CSS_TAG {
        ...
        Ok(oranda_generate_css::build_css(dist_dir)?)
    } else if maybe_env.is_some() && release_tag == ORANDA_CSS_TAG {
        // If we're in a release build, we can inline a file generated by oranda's build script.
        // Don't even build this if we're not in a release build so that Clippy et al don't complain
