...
pub fn dump_syntax_themes() -> Result<()> {
    ...
    let zon_syntax_file =
        std::fs::read_to_string("./src/site/markdown/syntax_highlight/Zon.sublime-syntax")?;
    ...
    let zon_syntax = SyntaxDefinition::load_from_str(&zon_syntax_file, true, Some("zon")).unwrap();
