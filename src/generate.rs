...
pub fn generate_ci(path: Utf8PathBuf, site_dir: &Option<Utf8PathBuf>) -> Result<()> {
    ...
    let append_gitignore = Confirm::new("Would you like to append the default destination directory (`public`) to your .gitignore file?")
        .with_default(false)
        .with_render_config(render_config)
        .prompt()
        .expect("Error while prompting!");
    if append_gitignore {
        append_dist_dir_to_gitignore()?;
    }
...
fn append_dist_dir_to_gitignore() -> Result<()> {
    let path = Utf8PathBuf::from(".gitignore");
    let mut contents = if path.exists() {
        LocalAsset::load_string(&path)?
    } else {
        String::new()
    };
    contents.push_str("\n# Generated by `oranda generate ci`\npublic/");
    LocalAsset::write_new(&contents, &path)?;
